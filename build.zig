...
pub fn build(b: *std.Build) void {
    ...
    const mod = b.addModule("my_first_zig", .{
    ...
    const exe = b.addExecutable(.{
        ...
        .root_module = b.createModule(.{
            ...
            // root module.
            .imports = &.{
                // Here "my_first_zig" is the name you will use in your source code to
                // import this module (e.g. `@import("my_first_zig")`). The name is
                // repeated because you are allowed to rename your imports, which
                // can be extremely useful in case of collisions (which can happen
                // importing modules from different packages).
                .{ .name = "my_first_zig", .module = mod },
            },
        }),
    });
    ...
    // Launch the bundled app through LaunchServices. Running the raw Mach-O
    // directly can fail during AppKit application registration on recent macOS.
    const run_cmd = b.addSystemCommand(&.{ "open", b.getInstallPath(.prefix, app_bundle_path) });
    run_step.dependOn(&run_cmd.step);

    // Make sure the app bundle is installed before LaunchServices opens it.
